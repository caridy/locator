...
function BundleLocator(options) {
    ...
    this.ready = new libpromise.Promise(function () {});
...
BundleLocator.prototype = {
    ...
    /**
     * Parses the folder denotated by the `applicationDirectory` configuration
     * from the constructor to turn it into the root bundle.
     ...
     */
    parseApplication: function (options) {
        this.parseBundle(this._options.applicationDirectory, options);
    },
    ...
    parseBundle: function (dir, options) {
        ...
        return this._walkNPMTree(dir).then(function (bundleSeeds) {
            ...
            bundleSeeds.forEach(function (bundleSeed) {
                var opts = {};
                if (bundleSeed.baseDirectory === dir) {
                    opts = options || {};
                    self._rootBundleName = bundleSeed.name;
                }
                ...
                return new libpromise.Promise(function (fulfill, reject) {
                    if (self._fileQueue.length) {
                        return self._processFileQueue().then(function () {
                            self._cacheBundle(self._bundles[self._rootBundleName]);
                            self.ready._resolver.fulfill();
                            fulfill(self._bundles[self._getBundleNameByPath(dir)]);
                        }, function (err) {
                            self.ready._resolver.reject(err);
                            reject(err);
                        });
                    }
                    // if there is no file in queue after the setup of the bundles,
                    // that indicates that everything is running off the cache :)
                    self.ready._resolver.fulfill();
                });
            });
        });
    ...
    _makeBundle: function (seed, parent) {
        ...
        return new libpromise.Promise(function (fulfill, reject) {
            ...
            if (self._options.buildDirectory) {
                bundle.buildDirectory = self._nameBuildDirectory(
                    bundle, self._options.buildDirectory);
    ...
    _walkBundle: function (bundleSeed) {
        ...
        return this._makeBundle(bundleSeed, parent).then(function (bundle) {
            ...
            return new libpromise.Promise(function (fulfill, reject) {
                ...
                // forcing to rehydrate (usually on production)
                if (self._options.rehydrate) {
                    return self._rehydrateBundle(bundle).then(fulfill, reject);
                }

                if (/*!bundle.baseDirectory.isLink() || */bundle.name !== self._rootBundleName) {
                    // attempting to rehydrate dependencies to speed up development,
                    // but supporting regular walk as a fallback
                    self._rehydrateBundle(bundle).then(fulfill, walk);
                } else {
                    // regular walk in case of symlinks or root bundle
                    walk();
                }
                function walk() {
                    // if rehydration is not available or failed,
                    // we should walk the bundle instead
                    walker = new ScanFS(null, function (err, fullPath) {
    ...
    /**
     * Rehydrates a bundle from cache.
     * @private
     * @method _rehydrateBundle
     * @param {Bundle} bundle The bundle
     * @return {Promise/A+} A promise that will be fulfilled if the bundle can be rehydrated
     * from cache.
     */
    _rehydrateBundle: function (bundle) {
        var self = this;

        return new libpromise.Promise(function (fulfill, reject) {
            var cacheFile = bundle.buildDirectory + '/.cache/locator.js',
                str,
                newBundle;
    
            try {
                // TODO: make this async
                str = libfs.readFileSync(cacheFile, 'utf8');
            } catch (e1) {
                reject(new Error('Unable to read cache data for bundle ' + bundle.name +
                        ' at ' + cacheFile));
            }
    
            // replacing tokens
            str = str.replace(new RegExp('@__buildDirectory__@', 'g'), bundle.buildDirectory);
            str = str.replace(new RegExp('@__baseDirectory__@', 'g'),  bundle.baseDirectory);
            // evaluating new locator content
            try {
                newBundle = libvm.runInThisContext('module = {};' + str, cacheFile);
            } catch (e2) {
                reject(new Error('Unable to restore bundle ' + bundle.name + ' from cached: ' + e2));
            }
            // using the cached bundle instead
            self._bundles[bundle.name] = newBundle;
            newBundle.cached = true;
            fulfill(newBundle);
        });
    },
    ...
    /**
     * Caches a bundle so it can be rehydrated later on.
     * @private
     * @method _cacheBundle
     * @param {Bundle} bundle The bundle
     * @return {Promise/A+} A promise that will be fulfilled if the bundle can be cached.
     */
    _cacheBundle: function (bundle) {
        var self = this;
        return new libpromise.Promise(function (fulfill, reject) {
            var cacheFile   = libpath.join(bundle.buildDirectory, '.cache/locator.js'),
                cache = serialize(bundle);

            // replacing tokens
            cache = cache.replace(new RegExp(bundle.buildDirectory, 'g'), '@__buildDirectory__@');
            cache = cache.replace(new RegExp(bundle.baseDirectory, 'g'),  '@__baseDirectory__@');
            cache = 'module.exports = ' + cache + ';';

            libmkdirp(libpath.dirname(cacheFile), null, function (err) {
                if (err) {
                    reject(err);
                } else {
                    libfs.writeFile(cacheFile, cache, 'utf8', function (err) {
                        if (err) {
                            reject(new Error('Unable to write cache data for bundle ' + 
                                    bundle.name + ' at ' + cacheFile + '; ' + err));
                        } else {
console.log(bundle.name, 'written in cache');
                            fulfill(cacheFile);
                        }
                    });
                }
